...
class DataBase:
    ...
    def __get_user(self, session: Session, user_model) -> UserModel:
        return session.query(UserModel).filter(UserModel.email.like(user_model.email)).first()

    def __get_product(self, session: Session, product_data) -> ProductModel:
        return session.query(ProductModel).filter(ProductModel.sku.like(product_data["sku"])).first()

    def __get_comment(self, session: Session, comment_data) -> CommentModel:
    ...
    def add_user(self, user_model, user_data):
        with Session(bind=self.engine) as session:
            if not self.__get_user(session, user_model):
    ...
    def add_product(self, user_model, product_data):
        with Session(bind=self.engine) as session:
            user = self.__get_user(session, user_model)

            if user and not self.__get_product(session, product_data):
    ...
    def add_comment(self, product_data, comment_data):
        with Session(bind=self.engine) as session:
            product = self.__get_product(session, product_data)
            
            if product and not self.__get_comment(session, comment_data):
